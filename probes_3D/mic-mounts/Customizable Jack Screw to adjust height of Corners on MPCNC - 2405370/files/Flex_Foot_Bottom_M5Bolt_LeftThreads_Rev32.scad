...
//Print Time: 2:59:11 at 60%infill 1.02oz
//
//-----------------------------------------
...
OD = 31.9;
Base_Ht = 1;        //Base Height to start Threads
Pit = 4;            //Pitch
Thread_Ht = 40.5;
Bot_Threads_ON = 1; //0 = No threads at bottom, just cylinder
Bot_OD = 30.9 + .4; //Diameter of bottom Cylinder if threads are not used
                    //Previous design used 30.9, but add .4mm for clearance fit
//-----------------------------------------
Len = 53;           //Base Length
Wid = Len;          //Base Width
Cyl_ID = 28.5;      //Diameter of EMT pipe
                    ...
                    //Nomimal OD is .922" or 23.4188mm
                    //Inside diameter .824" or 20.9296mm
                    //IE version was 25mm
...
//M5Head_Ht = 5.3;          //Height of Bolt Head to Recess into plastic
M5Nut_OD = 10;
M5_Res = 32;
Bolt_Len = 70;
...
module VerticalHoles()
{
    translate([Vert_Hole_Offset,Vert_Hole_Offset,0])
    {
        ...
        rotate([0,0,15])
        cylinder(d=M5Nut_OD,h=M5Head_Ht+1+50,$fn=M5_Res);
        ChamCorner();
    }
}
...
module DrawFinal()
{
    difference()
    {
        ...
        {
            translate([0,0,EMT_Base])
            cylinder(d=Bot_OD,h=Cyl_Ht+2,$fn=Bot_OD_Res);
        }
        translate([-NutX_Offset,1.5,NutZ_Height])
        rotate([-90,0,0])
